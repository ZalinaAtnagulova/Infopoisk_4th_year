...
def main(url):
    ...
    for article_link in articles_links:
        with open('Corpora/' + str(file_number) + '.txt', 'w', encoding = 'utf-8') as f:
...
def article_data(article, link):
    ...
    for b in range(100, 401, 10):
        for i, h in enumerate(soup.findAll('div', {'style':'float:left;margin:5px 20px 10px 0;width:'+str(b)+'px;'})):
