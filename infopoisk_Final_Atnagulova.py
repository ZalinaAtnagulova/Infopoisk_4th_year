###Final version with pymorphy instead of mystem and without repetitions in output
...
def output(out):
    ...
    for article in sorted(out, key=lambda n: out[n][1], reverse=True)[:10]:
        if len(titles) == 0:
            titles.append([out[article][0]])
            titles.append((out[article][0], out[article][2]))
        else:
            if out[article][0] not in titles[0]:
                titles[0].append(out[article][0])
                titles.append((out[article][0], out[article][2]))
    return titles[1:]
